#!/usr/bin/env python3
"""
Batch evaluation CLI for BioMaster cell type predictions.

Usage:
    # Run evaluation on all tissues
    python run_evaluation_biomaster.py

    # Run evaluation on one or more specific tissues
    python run_evaluation_biomaster.py --tissue brain
    python run_evaluation_biomaster.py --tissue brain breast

    # Skip LLM calls (use existing mappings only)
    python run_evaluation_biomaster.py --tissue brain --skip-llm

    # Disable plot generation
    python run_evaluation_biomaster.py --no-plots
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Batch evaluation pipeline for BioMaster cell type predictions',
...
Examples:
  ...
  %(prog)s --output-dir custom_results      # Custom output directory
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Root directory containing BioMaster outputs (default: auto-detect)'
    )

    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('evaluation_results_biomaster'),
        help='Directory to save evaluation results (default: evaluation_results_biomaster)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='cell_type',
        help='Column name for predictions in prediction files (default: cell_type)'
    )
...
def auto_detect_paths() -> tuple[Path, Path]:
    """
    Auto-detect data root and BioMaster output root paths.
    ...
    output_root = project_root / 'agent_outputs' / 'clustered' / 'biomaster'
...
def main() -> int:
    ...
    logger.info("=" * 60)
    logger.info("BIOMASTER CELL TYPE EVALUATION PIPELINE")
    logger.info("=" * 60)

    try:
        # Auto-detect paths if not provided
        if args.data_root is None or args.output_root is None:
            logger.info("Auto-detecting paths for BioMaster...")
