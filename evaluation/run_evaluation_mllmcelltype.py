#!/usr/bin/env python3
"""
Batch evaluation CLI for mLLMCellType predictions.

Usage:
    # Run evaluation on all tissues
    python run_evaluation_mllmcelltype.py

    # Run evaluation on one or more specific tissues
    python run_evaluation_mllmcelltype.py --tissue brain
    python run_evaluation_mllmcelltype.py --tissue brain breast

    # Skip LLM calls (use existing mappings only)
    python run_evaluation_mllmcelltype.py --tissue brain --skip-llm

    # Disable plot generation
    python run_evaluation_mllmcelltype.py --no-plots
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Batch evaluation pipeline for mLLMCellType predictions',
...
Examples:
  ...
  %(prog)s --output-dir custom_results      # Custom output directory
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Root directory containing agent outputs (default: auto-detect to mLLMCellType output)'
    )
    
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('evaluation_results_mllmcelltype'),
        help='Directory to save evaluation results (default: evaluation_results_mllmcelltype)'
    )
...
def auto_detect_paths() -> tuple[Path, Path]:
    """
    Auto-detect data root and output root paths for mLLMCellType.
    ...
    output_root = project_root / 'agent_outputs' / 'clustered' / 'mllmcelltype'
...
def main() -> int:
    ...
    logger.info("="*60)
    logger.info("mLLMCellType EVALUATION PIPELINE")
    logger.info("="*60)
    
    try:
        # Auto-detect paths if not provided
        if args.data_root is None or args.output_root is None:
            logger.info("Auto-detecting paths for mLLMCellType...")
        ...
        logger.info(f"Output root (mLLMCellType): {args.output_root}")
